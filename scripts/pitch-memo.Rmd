...
This memo makes the case for using fabricated data in writing analysis and describes two approaches for creating fabricated data sets.
...
Fabricated data has a different use case than synthetic data. Fabrication is meant to strength our research by *removing* any correlation between variables, while maintain the distributions of the data elements. Even with a clearly defined analysis plan, a researcher will be tempted to dig into interesting relationships that they find during analysis, straying from their plan, and introducing research bias. Using fabricated data, a researcher knows that they are unlikely any relationships. If they do find something interesting, they know it is spurious.
...
2.  Fabricated data set or sets[^1]
...
[^1]: Approach 1 is the better option here, because outliers and other data issues are more likely to be preserved. The data preparation will then need to be written to account for these issues.
